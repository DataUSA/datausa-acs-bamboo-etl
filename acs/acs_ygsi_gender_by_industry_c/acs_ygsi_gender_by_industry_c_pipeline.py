...
class TransformStep(PipelineStep):
    def run_step(self, prev, params):
        ...
        list_apis = [
            DICT_APIS['acs_ygsi_gender_by_industry_c_1'], 
            DICT_APIS['acs_ygsi_gender_by_industry_c_2']
        ]

        def transform_by_zone(year, geo, estimate, apis, api_key):
            df = read_file('/home/deploy/datausa-acs-bamboo-etl/acs/data/C24030_2014.csv') if str(year) == '2014' and estimate == '1' and geo == 'us' else read_by_zone(year, geo, estimate, apis, api_key)
...
class AcsYgsiGenderByIndustryCPipeline(EasyPipeline):
    @staticmethod
    def parameter_list():
        return[
    ...
    def steps(params):
        ...
        load_step = LoadStep(
            "acs_ygsi_gender_by_industry_c_{}".format(params.get('estimate')), db_connector, if_exists='append', dtype=dtype, pk=['geoid', 'year'], nullable_list=['mea', 'moe']
        )

        return [transform_step, load_step]
        
if __name__ == '__main__':
    acs_pipeline = AcsYgsiGenderByIndustryCPipeline()
    for estimate in ['1', '5']:
