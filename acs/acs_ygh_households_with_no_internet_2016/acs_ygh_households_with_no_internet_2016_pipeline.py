...
class TransformStep(PipelineStep):
    def run_step(self, prev, params):
        ...
        apis = DICT_APIS['acs_ygh_households_with_no_internet_2016']
...
class AcsYghHouseholdsWithNoInternet2016Pipeline(EasyPipeline):
    @staticmethod
    def parameter_list():
        return[
...
if __name__ == '__main__':
    acs_pipeline = AcsYghHouseholdsWithNoInternet2016Pipeline()
    for estimate in ['1', '5']:
        init = 2016 if estimate == '1' else 2017
        for year in range(init, 2019 + 1):
            acs_pipeline.run({
                'year': year,
                'estimate': estimate,
