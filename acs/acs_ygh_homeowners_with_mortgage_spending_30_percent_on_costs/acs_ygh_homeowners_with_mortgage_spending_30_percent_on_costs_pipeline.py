...
class TransformStep(PipelineStep):
    def run_step(self, prev, params):
        ...
        apis = DICT_APIS['acs_ygh_homeowners_with_mortgage_spending_30_percent_on_costs']

        def transform_by_zone(year, geo, estimate, apis, api_key):
            df = read_file('/home/deploy/datausa-acs-bamboo-etl/acs/data/B25092_2014.csv') if str(year) == '2014' and estimate == '1' and geo == 'us' else read_by_zone(year, geo, estimate, apis, api_key)
...
class AcsYghHomeownersWithMortgageSpending30PercentOnCostsPipeline(EasyPipeline):
    @staticmethod
    def parameter_list():
        return[
...
if __name__ == '__main__':
    acs_pipeline = AcsYghHomeownersWithMortgageSpending30PercentOnCostsPipeline()
    for estimate in ['1', '5']:
