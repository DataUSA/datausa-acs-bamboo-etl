...
class TransformStep(PipelineStep):
    def run_step(self, prev, params):
        ...
        apis = DICT_APIS['acs_ygh_occupied_households_lacking_plumbing']

        def transform_by_zone(year, geo, estimate, apis, api_key):
            df = read_file('/home/deploy/datausa-acs-bamboo-etl/acs/data/B25048_2014.csv') if str(year) == '2014' and estimate == '1' and geo == 'us' else read_by_zone(year, geo, estimate, apis, api_key)
        ...
        list_geo = ['us', 'state', 'county', 'place', 'public use microdata area', 'metropolitan statistical area/micropolitan statistical area', 'congressional district', 'tract'] if estimate == '1' else ['us', 'state', 'county', 'place', 'public use microdata area', 'metropolitan statistical area/micropolitan statistical area', 'tract', 'congressional district']
        ...
        df_final['dim_0'] = df_final['dim_0'].astype(int)
...
class AcsYghOccupiedHouseholdsLackingPlumbingPipeline(EasyPipeline):
    @staticmethod
    def parameter_list():
        return[
