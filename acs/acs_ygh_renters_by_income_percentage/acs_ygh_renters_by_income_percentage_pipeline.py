...
class TransformStep(PipelineStep):
    def run_step(self, prev, params):
        ...
        list_apis = [
            DICT_APIS['acs_ygh_renters_by_income_percentage_1'], 
            DICT_APIS['acs_ygh_renters_by_income_percentage_2'], 
            DICT_APIS['acs_ygh_renters_by_income_percentage_3']
        ]

        def transform_by_zone(year, geo, estimate, apis, api_key):
            df = read_file('/home/deploy/datausa-acs-bamboo-etl/acs/data/B25074_2014.csv') if str(year) == '2014' and estimate == '1' and geo == 'us' else read_by_zone(year, geo, estimate, apis, api_key)
...
class AcsYghRentersByIncomePercentagePipeline(EasyPipeline):
    @staticmethod
    def parameter_list():
        return[
...
if __name__ == '__main__':
    acs_pipeline = AcsYghRentersByIncomePercentagePipeline()
    for estimate in ['1', '5']:
