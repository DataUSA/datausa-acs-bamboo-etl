...
class TransformStep(PipelineStep):
    def run_step(self, prev, params):
        ...
        apis = DICT_APIS['acs_ygmt_mortgage_status_by_real_estate_taxes']

        def transform_by_zone(year, geo, estimate, apis, api_key):
            df = read_file('/home/deploy/datausa-acs-bamboo-etl/acs/data/B25102_2014.csv') if str(year) == '2014' and estimate == '1' and geo == 'us' else read_by_zone(year, geo, estimate, apis, api_key)
...
class AcsYgmtMortgageStatusByRealEstateTaxesPipeline(EasyPipeline):
    @staticmethod
    def parameter_list():
        return[
    ...
    def steps(params):
        ...
        load_step = LoadStep(
            "acs_ygmt_mortgage_status_by_real_estate_taxes_{}".format(params.get('estimate')), db_connector, if_exists='append',
...
if __name__ == '__main__':
    acs_pipeline = AcsYgmtMortgageStatusByRealEstateTaxesPipeline()
    for estimate in ['1', '5']:
